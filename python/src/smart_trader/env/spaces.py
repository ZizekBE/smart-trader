...
class SpaceConfig:
    ...
    microstructure_dim: int = 5  # spread, depth_imbalance, funding, oi_change, vol_profile
...
def build_action_space() -> spaces.Dict:
    ...
    Components:
      - regime:     Discrete(4)  — trending_up / trending_down / ranging / uncertain
...
def flatten_action(action: dict) -> np.ndarray:
    ...
    return np.concatenate([
        np.array([action["regime"]], dtype=np.float32),
...
def unflatten_action(flat: np.ndarray) -> dict:
    ...
    return {
        "regime": int(np.clip(np.round(flat[0]), 0, 3)),
        "position": np.clip(flat[1:2], -1, 1).astype(np.float32),
        "risk_budget": np.clip(flat[2:3], 0.01, 0.10).astype(np.float32),
        "hold_bars": int(np.clip(np.round(flat[3]), 0, 2)),
    }
